...
function updateBars() {
  ...
  document.getElementById("dragonHealth").style.width = (dragonHp / maxDragonHp * 100) + "%";
}

function checkGameOver() {
  if (snakeHp === 0 && lizardHp === 0 && dragonHp === 0) {
    log("🎉 You win the battle!");
    disableButtons();
    gameOver = true;
    if (confirm("Continue to next Cutscene")) {
        window.location.href = "Cutscene5.html";
    }
  } else if (hp === 0) {
...
function enemyAttack() {
  ...
  const damage = Math.floor(Math.random() * 10) + 7;
  hp -= damage;
  if (hp < 0) hp = 0;
...
function dealDamageToEnemy(enemyId, damage) {
  if (enemyId === 'snake') {
    ...
    log(`💢 Soul Keeper takes ${damage} damage!`);
    showEnemyDamageText('snake', damage);
  } else if (enemyId === 'lizard') {
    lizardHp -= damage;
    if (lizardHp < 0) lizardHp = 0;
    log(`💢 Soul Keeper takes ${damage} damage!`);
    showEnemyDamageText('lizard', damage);
  } else if (enemyId === 'dragon') {
    dragonHp -= damage;
    if (dragonHp < 0) dragonHp = 0;
    log(`💢 Dragon takes ${damage} damage!`);
    showEnemyDamageText('dragon', damage);
  }
...
function castFire() {
  if (mp >= 5 && !gameOver) {
    ...
    setTimeout(() => {
      dealDamageToEnemy('snake', 12);
      dealDamageToEnemy('lizard', 7);
      dealDamageToEnemy('dragon', 12);
      fireball.remove();
    }, 1000);
  } else {
    log("Not enough MP!");
...
function bladeOfTheAncients() {
  if (mp >= 20 && !gameOver) {
    ...
    dealDamageToEnemy('snake', 50);
    dealDamageToEnemy('lizard', 30);
    dealDamageToEnemy('dragon', 50);
